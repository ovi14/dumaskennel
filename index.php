<?php include('header.html') ?>

      <div class="mainbar">
        <div class="article">
          <h2><span>Welcome to the official website of <i>Duma's Kennel</i></span></h2>
          <p></p>
          <h3><span>Best quallity for mops/pug and french bulldog dogs</span></h3>
          <p></p>
        </div>
        <div class="article">
            <h3><i>Use the menu from the left to navigate through the site</i></h3>
            <h3><i>Click on the links of Mops/Pug or French Bulldog to find the pupppy that you wish</i></h3>
            <h3><i>You can go through all the litters, males and females that were until now and are planned</i></h3>
            <h3><i>You can click on each dog also to see it's pedigree</i></h3>
            <h3><i>Enjoy your visit and don't hesitate to make a call or write an email</i></h3>
        </div>
      </div>

      <div class="sidebar">
        <div class="gadget">
          <h2 class="star">Menu</h2>
          <ul class="sb_menu">
            <li class="active"><a href="index.php">Home</a></li>
            <li><a href="mops_pug.php">Mops-pug</a></li>
            <!--li><a href="french_bulldog.php">French bulldog</a></li-->
            <li><a href="about.php">About us</a></li>
            <li><a href="contact.php">Contact us</a></li>
          </ul>
        </div>
        <div class="gadget">
          <h2 class="star"><span>Recomended</span></h2>
          <ul class="ex_menu">
            <li><a href="http://www.artadresajuluicanin.ro" title="Profesional Dog training">Profesional Dog training</a><br />Profesioanl dog training in Timisoara and around</li>
            <li><a href="http://www.nik-majestic.com/" title="Canisa Nik Majestic">Canisa Nik Majestic</a><br />Canisa Nik Majestic - Tosa Inu</li>
          </ul>
        </div>
      </div>
      <div class="clr"></div>

    </div>
  </div>

<?php include('footer.html') ?>